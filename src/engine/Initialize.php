...
use ACF_Business_Directory\Engine;

/**
 * ACF_Business_Directory Initializer
 */
class Initialize {
	...
	private function load_classes() {
		$this->classes = \apply_filters( 'acf_business_directory_classes_to_execute', $this->classes );

		foreach ( $this->classes as $class ) {
			try {
				$this->initialize_plugin_class( $class );
			} catch ( \Throwable $err ) {
				\do_action( 'acf_business_directory_initialize_failed', $err );
	...
	private function get_classes( string $namespacetofind ) {
		...
		$namespacetofind = 'ACF_Business_Directory\\' . $namespacetofind;

		// In case composer has autoload optimized
		if ( isset( $classmap[ 'ACF_Business_Directory\\Engine\\Initialize' ] ) ) {
		...
		if ( isset( $prefix[ $namespacetofind ] ) ) {
			...
			if ( !WP_DEBUG ) {
				\wp_die( \esc_html__( 'ACF Business Directory is on production environment with missing `composer dumpautoload -o` that will improve the performance on autoloading itself.', ABD_TEXTDOMAIN ) );
			}
